...
def get_vgg16_state_dict():
    model = models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1)
    return copy.deepcopy(model.state_dict())

@st.cache_resource(show_spinner=False)
def get_vgg19_state_dict():
    model = models.vgg19(weights=models.VGG19_Weights.IMAGENET1K_V1)
    return copy.deepcopy(model.state_dict())

def build_fresh_vgg16():
    m = models.vgg16(weights=None)
    m.load_state_dict(get_vgg16_state_dict())
    m.eval(); return m

def build_fresh_vgg19():
    m = models.vgg19(weights=None)
    m.load_state_dict(get_vgg19_state_dict())
    m.eval(); return m
...
SHARED_LAYERS = [
    "Block 1 – Conv1 (64ch)", "Block 1 – Conv2 (64ch)", "Block 1 – MaxPool",
    "Block 2 – Conv1 (128ch)", "Block 2 – Conv2 (128ch)", "Block 2 – MaxPool",
    "Block 3 – Conv1 (256ch)", "Block 3 – Conv2 (256ch)", "Block 3 – MaxPool",
    "Block 4 – Conv1 (512ch)", "Block 4 – Conv2 (512ch)", "Block 4 – MaxPool",
    "Block 5 – Conv1 (512ch)", "Block 5 – Conv2 (512ch)", "Block 5 – MaxPool",
]
...
def preprocess_image(pil_img):
    try:
        return IMAGE_TRANSFORM(pil_img.convert("RGB")).unsqueeze(0)
    except Exception as e:
        st.error(f"Image preprocessing failed: {e}"); raise

def extract_activations(model, tensor, layer_idx):
    activation = {}
...
def run_full_inference(model, tensor):
    with torch.no_grad():
        logits = model(tensor)
    return torch.softmax(logits, dim=1)[0].numpy()

def render_feature_maps(activations, n_maps, title, colormap="viridis"):
    n_maps = min(n_maps, activations.shape[0])
    ncols  = min(8, n_maps)
    nrows  = (n_maps + ncols - 1) // ncols
    fig, axes = plt.subplots(nrows, ncols,
                              figsize=(ncols*1.9, nrows*1.9), facecolor="#f8fafc")
    fig.suptitle(title, fontsize=10, fontweight="bold", color="#1e3a5f", y=1.01)
    axes_flat = ([axes] if n_maps==1 else list(axes) if nrows==1
...
# SECTION A
# ──────────────────────────────────────────────
def section_feature_explorer():
    st.markdown('<div class="section-title">🔬 Live Feature Map Explorer</div>',
                unsafe_allow_html=True)
    st.markdown("""
    <div class="card card-blue">
    <b>Upload an image</b> and explore how VGG-16 and VGG-19 transform it through their
    convolutional layers. Use the controls below to select layers, compare models, and
    inspect feature maps in real time.
    ...
    with col_ctrl:
        st.markdown('<div class="subsection-title">⚙️ Explorer Controls</div>',
                    unsafe_allow_html=True)
        mode = st.radio("Mode", ["Single Model",
                                  "Compare VGG-16 vs VGG-19 Side-by-Side",
                                  "Before / After MaxPool"], index=0)
        if mode == "Single Model":
            model_choice = st.selectbox("Select Model", ["VGG-16","VGG-19"])
            layer_map  = VGG16_LAYER_MAP if model_choice=="VGG-16" else VGG19_LAYER_MAP
            ...
            model_choice = st.selectbox("Select Model", ["VGG-16","VGG-19"])
            pool_layers  = {k:v for k,v in
                            (VGG16_LAYER_MAP if model_choice=="VGG-16"
                             else VGG19_LAYER_MAP).items() if "MaxPool" in k}
    ...
    if mode == "Single Model":
        with st.spinner(f"Running {model_choice} forward pass…"):
        ...
        _show_activation_stats(acts, layer_name, model_choice)
        with st.spinner("Rendering feature maps…"):
            fig = render_feature_maps(acts, n_maps,
                                      f"{model_choice} — {layer_name}", cmap_choice)
            st.image(fig_to_bytes(fig), use_container_width=True); plt.close(fig)
        _show_top3_predictions(probs, layer_name, model_choice)
...
def _show_hook_expander():
    with st.expander("🔧 How Forward Hooks Work (Annotated Code)"):
        ...
        <b>The Hook Mechanism</b> — PyTorch allows you to intercept any layer's output
        during a forward pass using <code>register_forward_hook</code>.
        </div>""", unsafe_allow_html=True)
        st.code('''
activation = {}
def hook_fn(module, input, output):
...
def _apply_conv(img, filt, pad=1):
    ...
    H, W   = img.shape; fh, fw = filt.shape
    out    = np.zeros((H, W))
    for r in range(H):
        for c in range(W):
...
def story_step1():
    ...
    The filter slides <strong>left-to-right, top-to-bottom</strong>. At every position it
    extracts a 3×3 patch, performs an <b>element-wise multiply</b>, then <b>sums</b>
...
def render_sidebar():
    with st.sidebar:
        ...
        <b style="color:#7dd3fc">About this App</b><br><br>
        • Live feature map extraction<br>
        • Forward hook mechanism<br>
        • Interactive 8×8 matrix demos<br>
        • Receptive field growth<br>
        • Parameter efficiency comparisons<br><br>
        <b style="color:#7dd3fc">Models</b><br>
        VGG-16 &amp; VGG-19 pretrained on ImageNet<br><br>
